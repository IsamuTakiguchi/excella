/**
 * ExcelJS を使った .xlsx ⇄ WorkbookModel の変換（バイト列 ⇄ モデル）。
 *
 * ファイルシステムには触らないので、Electron の main プロセスからも
 * ブラウザ（PWA）からも同じコードで使える。ファイルの読み書きは
 * main 側（src/main/io/xlsx.ts）とブラウザ側（src/renderer/webBridge.ts）が
 * それぞれの手段で行う。
 *
...
import { addrToA1, a1ToAddr } from './a1'
import {
  createSheet,
  DEFAULT_COL_COUNT,
  DEFAULT_ROW_COUNT,
...
/** xlsx のバイト列をモデルに変換する */
export async function workbookFromXlsxBuffer(
  data: ArrayBuffer | Uint8Array,
): Promise<WorkbookModel> {
  const wb = new ExcelJS.Workbook()
  // ExcelJS の型は Node の Buffer を要求するが、実装は Uint8Array / ArrayBuffer で動く
  // （ブラウザにも Buffer は無い）ので、型だけ緩めて渡す
  const xlsx = wb.xlsx as unknown as { load(input: ArrayBuffer | Uint8Array): Promise<unknown> }
  await xlsx.load(data)
...
/** シート ID → (A1 → 計算結果) */
export type XlsxResults = Map<string, Map<string, string | number | boolean>>

/**
 * モデルを xlsx のバイト列にする。
 * `results` にシート ID → (A1 → 計算結果) を渡すと、数式の計算済み結果を
 * result として埋め込むので、Excel で開いた直後から値が表示される。
 */
export async function xlsxBufferFromWorkbook(
  model: WorkbookModel,
  results?: XlsxResults,
): Promise<Uint8Array> {
  const wb = new ExcelJS.Workbook()
